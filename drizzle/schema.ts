...
export const epgSources = mysqlTable("epgSources", { id: int("id").autoincrement().primaryKey(), name: varchar("name", { length: 140 }).notNull(), feedUrl: text("feedUrl"), status: mysqlEnum("status", ["healthy", "attention", "inactive"]).default("inactive").notNull(), programmeCount: int("programmeCount").default(0).notNull(), lastSyncedAt: timestamp("lastSyncedAt"), lastAttemptAt: timestamp("lastAttemptAt"), lastError: text("lastError"), coverageEndsAt: timestamp("coverageEndsAt"), refreshThresholdHours: int("refreshThresholdHours").default(6).notNull(), createdAt: timestamp("createdAt").defaultNow().notNull(), updatedAt: timestamp("updatedAt").defaultNow().onUpdateNow().notNull() });
