...
export const customers = mysqlTable("customers", {
  id: int("id").autoincrement().primaryKey(), label: varchar("label", { length: 90 }).notNull(), plan: varchar("plan", { length: 64 }).notNull(), planId: int("planId"), planCycleId: int("planCycleId"), screenLimit: int("screenLimit").default(1).notNull(), usedScreens: int("usedScreens").default(0).notNull(), expiresAt: timestamp("expiresAt").notNull(), trialEndsAt: timestamp("trialEndsAt"), accessTokenHash: varchar("accessTokenHash", { length: 64 }).notNull().unique(), status: mysqlEnum("status", ["active", "attention", "expired"]).default("active").notNull(), createdAt: timestamp("createdAt").defaultNow().notNull(), updatedAt: timestamp("updatedAt").defaultNow().onUpdateNow().notNull(),
});

export const channels = mysqlTable("channels", { id: int("id").autoincrement().primaryKey(), name: varchar("name", { length: 140 }).notNull(), groupTitle: varchar("groupTitle", { length: 90 }).notNull(), qualities: varchar("qualities", { length: 120 }).notNull(), ageRating: int("ageRating").default(0).notNull(), routeCount: int("routeCount").default(1).notNull(), isActive: boolean("isActive").default(false).notNull(), createdAt: timestamp("createdAt").defaultNow().notNull(), updatedAt: timestamp("updatedAt").defaultNow().onUpdateNow().notNull() });
export const channelSources = mysqlTable("channelSources", { id: int("id").autoincrement().primaryKey(), channelId: int("channelId").notNull(), quality: varchar("quality", { length: 24 }).notNull(), primaryUrl: text("primaryUrl").notNull(), fallbackUrl: text("fallbackUrl"), createdAt: timestamp("createdAt").defaultNow().notNull(), updatedAt: timestamp("updatedAt").defaultNow().onUpdateNow().notNull() });
export const epgSources = mysqlTable("epgSources", { id: int("id").autoincrement().primaryKey(), name: varchar("name", { length: 140 }).notNull(), status: mysqlEnum("status", ["healthy", "attention", "inactive"]).default("inactive").notNull(), programmeCount: int("programmeCount").default(0).notNull(), lastSyncedAt: timestamp("lastSyncedAt"), createdAt: timestamp("createdAt").defaultNow().notNull(), updatedAt: timestamp("updatedAt").defaultNow().onUpdateNow().notNull() });
