...
export default function VodPage() {
  ...
  const createVod = trpc.vod.create.useMutation({
    onSuccess: async () => {
      ...
      setTitle(""); setYear(""); setSourceUrl(""); setSynopsis("");
    },
  ...
  const handleMetadataSearch = () => {
    toast.message("Fonte de metadados ainda não conectada.", {
      description: "Quando a chave da fonte for configurada no servidor, a busca preencherá detalhes e capa sem expor a credencial no aplicativo.",
    });
  };
  ...
  const handleSave = () => {
    ...
    createVod.mutate({ title: title.trim(), kind, releaseYear: parsedYear, sourceUrl: sourceUrl.trim() || null, synopsis: synopsis.trim() || null });
  };

  return (
    <PanelLayout>
      <section className="relative overflow-hidden rounded-[26px] border border-white/[0.08] bg-[#10191f] p-6 sm:p-8">
        <img src="/manus-storage/nexus-epg-archive_68394a10.jpg" alt="" className="pointer-events-none absolute right-0 top-0 h-full w-1/2 object-cover opacity-[0.16] mix-blend-screen" />
      ...
      <section className="mt-6 grid gap-6 xl:grid-cols-[minmax(0,1.38fr)_minmax(340px,0.72fr)]">
        <article className="surface-card p-5 sm:p-6">
          <div className="flex flex-wrap items-start justify-between gap-4">
            ...
            <button type="button" onClick={handleMetadataSearch} className="pressable inline-flex items-center gap-2 rounded-xl border border-[#43E6C2]/20 bg-[#43E6C2]/[0.07] px-3 py-2 text-xs font-bold text-[#9df6df] transition hover:bg-[#43E6C2]/[0.13]"><Search className="h-3.5 w-3.5" /> Buscar metadados</button>
          </div>
        ...
        <div className="space-y-6">
          <article className="surface-card overflow-hidden">
            <div className="relative min-h-[170px] overflow-hidden border-b border-white/[0.07] bg-[#0b1519] p-5">
              <img src="/manus-storage/nexus-streamfield-wide_7bf781f7.jpg" alt="" className="pointer-events-none absolute inset-0 h-full w-full object-cover opacity-[0.18]" />
              <div className="relative flex h-full flex-col justify-between">
                ...
                <div><p className="font-mono text-[10px] uppercase tracking-[0.16em] text-slate-500">Imagem de capa</p><p className="mt-1 text-sm font-bold text-white">Aguardando fonte configurada</p></div>
              </div>
            </div>
            <div className="p-5">
              <p className="text-sm leading-6 text-slate-400">A fonte externa poderá trazer poster, backdrop, elenco, gêneros e descrição em português. A chave nunca será entregue ao React ou ao aplicativo.</p>
      ...
      <section className="mt-6 surface-card overflow-hidden">
        ...
        {catalogQuery.isLoading ? (
          ...
          <div className="divide-y divide-white/[0.06]">
            {catalog.map(item => (
              <div key={item.id} className="flex flex-wrap items-center gap-4 px-5 py-4 sm:px-6"><div className="grid h-9 w-9 place-items-center rounded-lg border border-white/[0.08] bg-white/[0.04]"><Sparkles className="h-3.5 w-3.5 text-[#43E6C2]" /></div><div className="min-w-[180px] flex-1"><p className="text-sm font-bold text-slate-100">{item.title}</p><p className="mt-1 font-mono text-[10px] uppercase tracking-[0.12em] text-slate-500">{getVodKindLabel(item.kind)} · {item.releaseYear ?? "—"}</p></div><span className="rounded-full border border-white/[0.1] bg-white/[0.03] px-2.5 py-1 font-mono text-[9px] uppercase tracking-[0.12em] text-slate-400">{item.status === "ready" ? "Pronto para publicar" : "Rascunho"}</span></div>
            ))}
          </div>
        )}
      </section>
