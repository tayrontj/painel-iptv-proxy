...
async function activeCredential(provider: "mercado_pago" | "vod_metadata") {
  ...
  if (!setting?.enabled || !setting.secretCiphertext || !setting.secretIv || !setting.secretTag) {
    throw new Error(`Integração ${provider === "mercado_pago" ? "Mercado Pago" : "de metadados VOD"} não configurada`);
  }
  return {
    baseUrl: setting.baseUrl,
    secret: decryptIntegrationSecret({ ciphertext: setting.secretCiphertext, iv: setting.secretIv, tag: setting.secretTag }),
  };
}

function integrationUrl(baseUrl: string | null, relativePath: string) {
  const endpoint = new URL(baseUrl || "");
  if (endpoint.protocol !== "https:") throw new Error("A URL da integração precisa utilizar HTTPS");
  return new URL(relativePath, endpoint.href.endsWith("/") ? endpoint.href : `${endpoint.href}/`);
}
...
export async function searchVodMetadata(query: string): Promise<MetadataResult[]> {
  ...
  return (payload.results ?? []).map(item => {
    const title = item.title || item.name || "Título sem identificação";
    const date = item.release_date || item.first_air_date || "";
    return {
      providerId: `metadata-${item.id}`,
      title,
...
export async function createMercadoPagoPix(input: { amountCents: number; description: string; payerEmail: string; externalReference: string }) {
  ...
  return {
    ...
    ticketUrl: payload.point_of_interaction?.transaction_data?.ticket_url ?? null,
  };
}
