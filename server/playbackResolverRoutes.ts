...
import { acquirePlaybackSession, getChannelSources, getVodEpisodeById, getVodItemById } from "./db";
...
function screenLimitReached(reservation: { activeSessions: number; screenLimit: number }) { return { error: { code: "GLOBAL_SCREEN_LIMIT_REACHED", message: "Todas as telas deste plano já estão em uso. A reprodução atual continua ativa; aumente o plano para assistir em mais telas.", activeScreens: reservation.activeSessions, screenLimit: reservation.screenLimit, upgrade: { action: "open_plan_options", procedure: "app.planOptions", label: "Ver planos e aumentar telas" } } }; }
...
export function registerPlaybackResolverRoutes(app: Express) {
  app.post("/api/internal/playback/resolve", async (req, res) => {
    ...
    const ticket = typeof req.body?.ticket === "string" ? req.body.ticket : ""; const nestedResource = typeof req.body?.resource === "string" ? req.body.resource : undefined; const consumerKeyHash = typeof req.body?.consumerKeyHash === "string" ? req.body.consumerKeyHash : "";
    if (nestedResource && nestedResource.length > 2048) return res.status(400).json({ error: "Recurso inválido" });
    const payload = await readPlaybackTicket(ticket); if (!payload || payload.consumerKeyHash !== consumerKeyHash) return res.status(401).json({ error: "Ticket de reprodução inválido, expirado ou usado por outro dispositivo" });
    try {
      const reservation = await acquirePlaybackSession({ customerId: payload.customerId, consumerKeyHash });
      if (!reservation.allowed) return res.status(429).json(screenLimitReached(reservation));
      const refreshedTicket = await createPlaybackTicket(payload);
      if (payload.resource === "channel") { const source = selectChannelRoute(await getChannelSources(payload.itemId), payload.quality); if (!source) return res.sendStatus(404); return res.json({ ticket: refreshedTicket, url: resolveNestedResource(source.url, nestedResource), headers: { ...(source.origin ? { Origin: source.origin } : {}), ...(source.referer ? { Referer: source.referer } : {}), "User-Agent": "VidelisMediaGateway/1.0" } }); }
      const item = payload.resource === "episode" ? await getVodEpisodeById(payload.itemId) : await getVodItemById(payload.itemId); if (!item?.sourceUrl) return res.sendStatus(404); return res.json({ ticket: refreshedTicket, url: resolveNestedResource(item.sourceUrl, nestedResource), headers: { "User-Agent": "VidelisMediaGateway/1.0" } });
