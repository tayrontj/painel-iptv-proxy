...
export const appRouter = router({
  ...
  auth: router({
    ...
    login: publicProcedure.input(z.object({ username: z.string().trim().min(3).max(64), password: z.string().min(1).max(256) })).mutation(async ({ input, ctx }) => { const admin = await db.authenticateAdmin(input.username, input.password); if (!admin) throw new TRPCError({ code: "UNAUTHORIZED", message: "Usuário ou senha inválidos." }); const session = await sdk.createSessionToken(admin.openId, { name: admin.name || "Administrador Nexus" }); ctx.res.cookie(COOKIE_NAME, session, getSessionCookieOptions(ctx.req)); return { success: true } as const; }),
